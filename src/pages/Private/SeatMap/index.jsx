...
const Seat = ({ seat, status, isSelected, onSelect }) => {
  const baseClasses =
    "w-8 h-8 md:w-10 md:h-10 rounded-t-lg flex items-center justify-center font-bold text-xs cursor-pointer transition-colors";

  let statusClasses = "bg-gray-600 hover:bg-red-500 text-white";
  if (status === "occupied") {
    statusClasses =
      "bg-gray-400 dark:bg-gray-700 cursor-not-allowed text-gray-800 dark:text-gray-500";
  }
  if (isSelected) {
    statusClasses =
      "bg-red-600 text-white ring-2 ring-offset-2 ring-offset-gray-900 ring-red-500";
  }
...
function SeatMap() {
  ...
  const handleSelectSeat = (seatLabel) => {
    setSelectedSeats((prevSelected) =>
      prevSelected.includes(seatLabel)
        ? prevSelected.filter((s) => s !== seatLabel)
  ...
  const handleProceedToPurchase = () => {
    ...
    // Navega para a nova rota, passando o estado
    navigate("/checkout", {
      state: {
        sessionId: sessionId,
        ...
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-400 flex items-center gap-2">
              ...
              {room.name} | <ClockIcon className="h-4 w-4" />
              ...
              >
                ...
                {row.map((seat) => (
                  <Seat
                    ...
                    seat={seat}
                    status={
                      occupiedSeats.includes(seat.label)
                        ? "occupied"
                        : "available"
                    }
          ...
          <div className="w-full max-w-3xl bg-gray-800/50 backdrop-blur-sm p-4 rounded-lg flex flex-col md:flex-row justify-between items-center gap-4 sticky bottom-4 border border-gray-700">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-t-lg bg-gray-600"></div>
                <span>Disponível</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-t-lg bg-red-600"></div>
                <span>Selecionado</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-5 h-5 rounded-t-lg bg-gray-700"></div>
                <span>Ocupado</span>
              </div>
            </div>
            <button
